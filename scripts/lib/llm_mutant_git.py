...
def changed_details(diff: str) -> dict[str, tuple[set[int], str]]:
    details: dict[str, tuple[set[int], list[str]]] = {}
    ...
    for line in diff.splitlines():
        if line.startswith("+++ b/"):
            current = line[6:]
            details.setdefault(current, (set(), []))
            ...
            details[current][0].add(old_line)
            details[current][1].append(line[1:])
            old_line += 1
        elif current and line.startswith("+") and not line.startswith("+++"):
            details[current][0].add(new_line)
            details[current][1].append(line[1:])
            new_line += 1
        elif current and line.startswith(" "):
            old_line += 1
            new_line += 1
    return {name: (locations, "\n".join(source)) for name, (locations, source) in details.items()}
