...
class TaskSelectionTest(unittest.TestCase):
    ...
    def test_requires_an_explicit_resume_selection(self) -> None:
        selection = select_task(self.tasks, "resume:task-b")
    ...
    def test_rejects_missing_or_unknown_selection(self) -> None:
        for choice in (None, "", "task-missing", "resume:task-missing"):
    ...
    def test_rejects_ambiguous_candidate_identity(self) -> None:
        with self.assertRaises(TaskSelectionError):
            select_task([self._task("task-a"), self._task("task-a")], "resume:task-a")

    def test_rejects_duplicate_candidates_before_any_selection(self) -> None:
        tasks = [self._task("task-a"), self._task("task-a"), self._task("task-b")]

        with self.assertRaises(TaskSelectionError):
            select_task(tasks, "resume:task-b")

    def test_allows_a_task_named_new_through_the_resume_tag(self) -> None:
        selection = select_task([self._task("new")], "resume:new")

        self.assertEqual(selection.task.task_id, "new")
